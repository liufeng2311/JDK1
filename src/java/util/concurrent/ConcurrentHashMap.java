 ...
 * block, so may overlap with update operations (including {@code put} and {@code remove}).
 * Retrievals reflect the results of the most recently <em>completed</em> update operations holding
 * upon their onset. (More formally, an update operation for a given key bears a
 * <em>happens-before</em> relation with any (non-null) retrieval for
 * that key reporting the updated value.)  For aggregate operations such as {@code putAll} and
 * {@code clear}, concurrent retrievals may reflect insertion or removal of only some entries.
 * Similarly, Iterators, Spliterators and Enumerations return elements reflecting the state of the
 * hash table at some point at or since the creation of the iterator/enumeration.  They do
 * <em>not</em> throw {@link java.util.ConcurrentModificationException
 * ConcurrentModificationException}. However, iterators are designed to be used by only one thread
 * at a time. Bear in mind that the results of aggregate status methods including {@code size},
 * {@code isEmpty}, and {@code containsValue} are typically useful only when a map is not undergoing
 ...
 * collisions (i.e., keys that have distinct hash codes but fall into the same slot modulo the table
 * size), with the expected average effect of maintaining roughly two bins per mapping
 * (corresponding to a 0.75 load factor threshold for resizing). There may be much variance around
 * this average as mappings are added and removed, but overall, this maintains a commonly accepted
 * time/space tradeoff for hash tables.  However, resizing this or any other kind of hash table may
 * be a relatively slow operation. When possible, it is a good idea to provide a size estimate as an
 * optional {@code initialCapacity} constructor argument. An additional optional {@code loadFactor}
 * constructor argument provides a further means of customizing initial table capacity by specifying
 * the table density to be used in calculating the amount of space to allocate for the given number
 * of elements.  Also, for compatibility with previous versions of this class, constructors may
 * optionally specify an expected {@code concurrencyLevel} as an additional hint for internal
 * sizing.  Note that using many keys with exactly the same {@code hashCode()} is a sure way to slow
 * down performance of any hash table. To ameliorate impact, when keys are {@link Comparable}, this
 * class may use comparison order among keys to help break ties.
 *
 * <p>A {@link Set} projection of a ConcurrentHashMap may be created
 * (using {@link #newKeySet()} or {@link #newKeySet(int)}), or viewed (using {@link #keySet(Object)}
 * when only keys are of interest, and the mapped values are (perhaps transiently) not used or all
 * take the same mapping value.
 *
 * <p>A ConcurrentHashMap can be used as scalable frequency map (a
 * form of histogram or multiset) by using {@link java.util.concurrent.atomic.LongAdder} values and
 * initializing via {@link #computeIfAbsent computeIfAbsent}. For example, to add a count to a
 * {@code ConcurrentHashMap<String,LongAdder> freqs}, you can use {@code freqs.computeIfAbsent(k ->
 ...
 * operations that, unlike most {@link Stream} methods, are designed to be safely, and often
 * sensibly, applied even with maps that are being concurrently updated by other threads; for
 * example, when computing a snapshot summary of the values in a shared registry. There are three
 * kinds of operation, each with four forms, accepting functions with Keys, Values, Entries, and
 * (Key, Value) arguments and/or return values. Because the elements of a ConcurrentHashMap are not
 * ordered in any particular way, and may be processed in different orders in different parallel
 * executions, the correctness of supplied functions should not depend on any ordering, or on any
 * other objects or values that may transiently change while computation is in progress; and except
 * for forEach actions, should ideally be side-effect-free. Bulk operations on {@link
 * java.util.Map.Entry} objects do not support method {@code setValue}.
 *
 * <ul>
 ...
 * @param <V> the type of mapped values 测试测试测试,终于可以做笔记啦
 * @author Doug Lea
 * @since 1.5
 */
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
   ...
   * @param initialCapacity  the initial capacity. The implementation performs internal sizing to
   *                         accommodate this many elements, given the specified load factor.
   * @param loadFactor       the load factor (table density) for establishing the initial table
   *                         size
  ...
  final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    for (Node<K, V>[] tab = table; ; ) {
      ...
      if (tab == null || (n = tab.length) == 0) {
        tab = initTable();
      } else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {   //通过getObjectVolatile()方法存从主存中获取tab[i]的最新值
        ...
        V oldVal = null;
        synchronized (f) {
          if (tabAt(tab, i) == f) {  //主存中的tab[i]节点可能发生变化(扩容等)
            if (fh >= 0) {
              binCount = 1;
   ...
   * and {@code clear} operations.  It does not support the {@code add} or {@code addAll}
   * operations.
   *
   * <p>The view's iterators and spliterators are
   * <a href="package-summary.html#Weakly"><i>weakly consistent</i></a>.
  ...
  public void forEach(BiConsumer<? super K, ? super V> action) {
    ...
    Node<K, V>[] t;
    if ((t = table) != null) {
      Traverser<K, V> it = new Traverser<K, V>(t, t.length, 0, t.length);  //保存当前数组的信息
      for (Node<K, V> p; (p = it.advance()) != null; ) {
   ...
   * using the given mapping function and enters it into this map unless {@code null}.  The entire
   ...
   * and its current mapped value.  The entire method invocation is performed atomically. Some
   * attempted update operations on this map by other threads may be blocked while computation is in
  ...
  /**
   * If the specified key is not already associated with a (non-null) value, associates it with the
   * given value. Otherwise, replaces the value with the results of the given remapping function, or
   * removes if {@code null}. The entire method invocation is performed atomically.  Some attempted
   * update operations on this map by other threads may be blocked while computation is in progress,
  ...
  static class Traverser<K, V> {
    ...
    Node<K, V>[] tab;        // 当前数组
    Node<K, V> next;         // 下一个要用到的节点
    TableStack<K, V> stack, spare; // to save/restore on ForwardingNodes
    int index;              // 遍历数组时使用的索引,从0开始
    int baseIndex;          // 数组的起始索引0
    int baseLimit;          // 数组的边界length-1
    final int baseSize;     // 数组的长度length
    ...
    final Node<K, V> advance() {
      ...
      for (; ; ) {
        ...
        if ((e = tabAt(t, i)) != null && e.hash < 0) {
          if (e instanceof ForwardingNode) {  //元素已被转移,需要从新数组上转移
            tab = ((ForwardingNode<K, V>) e).nextTable;
            e = null;
            pushState(t, i, n);
            continue;
          } else if (e instanceof TreeBin)  //当前节点为TreeBin,通过first链表找
          {
  ...
  /**
   * Computes initial batch value for bulk tasks. The returned value is approximately exp2 of the
   * number of times (minus one) to split task by two before executing leaf action. This value is
   * faster to compute and more convenient to use as a guide to splitting than is the depth, since
  ...
  /**
   * Returns a non-null result from applying the given search function on each (key, value), or null
   * if none.  Upon success, further element processing is suppressed and the results of any other
  ...
  /**
   * Returns the result of accumulating the given transformation of all (key, value) pairs using the
   ...
   * Upon success, further element processing is suppressed and the results of any other parallel
  ...
  /**
   * Returns the result of accumulating all keys using the given reducer to combine values, or null
   * if none.
   *
   * @param parallelismThreshold the (estimated) number of elements needed for this operation to be
   *                             executed in parallel
  ...
  /**
   * Returns the result of accumulating the given transformation of all keys using the given reducer
  ...
  /**
   * Returns a non-null result from applying the given search function on each value, or null if
   * none.  Upon success, further element processing is suppressed and the results of any other
   * parallel invocations of the search function are ignored.
   *
   * @param parallelismThreshold the (estimated) number of elements needed for this operation to be
   *                             executed in parallel
  ...
  /**
   * Returns the result of accumulating all values using the given reducer to combine values, or
   * null if none.
   *
   * @param parallelismThreshold the (estimated) number of elements needed for this operation to be
   *                             executed in parallel
  ...
  /**
   * Returns the result of accumulating the given transformation of all values using the given
  ...
  /**
   * Returns a non-null result from applying the given search function on each entry, or null if
   * none.  Upon success, further element processing is suppressed and the results of any other
   * parallel invocations of the search function are ignored.
   *
   * @param parallelismThreshold the (estimated) number of elements needed for this operation to be
  ...
  /**
   * Returns the result of accumulating all entries using the given reducer to combine values, or
   * null if none.
   *
   * @param parallelismThreshold the (estimated) number of elements needed for this operation to be
   *                             executed in parallel
  ...
  /**
   * Returns the result of accumulating the given transformation of all entries using the given
   * reducer to combine values, or null if none.
   *
   * @param parallelismThreshold the (estimated) number of elements needed for this operation to be
   *                             executed in parallel
   ...
   * #keySet() keySet()}, {@link #keySet(Object) keySet(V)}, {@link #newKeySet() newKeySet()},
  ...
  public static class KeySetView<K, V> extends CollectionView<K, V, K>
     ...
     * @return {@code true} if this set changed as a result of the call
     * @throws NullPointerException          if the collection or any of its elements are {@code
     *                                       null}
     * @throws UnsupportedOperationException if no default mapped value for additions was provided
     */
  ...
  /**
   * Base class for bulk tasks. Repeats some fields and code from class Traverser, because we need
   * to subclass CountedCompleter.
   */
